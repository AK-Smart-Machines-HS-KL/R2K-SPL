...
class WhistleRecognizer : public WhistleRecognizerBase
{
  STREAMABLE(Signature,
    ...
    (unsigned)timestamp, /**< The timestamp when the whistle was detected. */
    });

  std::vector<Signature> signatures; /**< All whistle signatures. */
  std::vector<Signature> bestSignatures; /*< All collected signatures until PLAYING*/
  std::vector<Vector2d> trueWhistle; /* the true whislte according to timedifference with Gamecontroller*/
  ...
  float correlate(std::vector<Vector2d>& signature, const RingBuffer<AudioData::Sample>& buffer, bool record = false);
