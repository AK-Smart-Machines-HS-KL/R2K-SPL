...
// field_of_view: Der Bereich in dem sich die Arme bewegen können in Grad
...
bool is_in_field_of_view(double pos1[2], double pos2[2], double rotation, double field_of_view) {
    ...
    double angle_object  = fmod((atan2(dy, dx) * (180.0 / M_PI) + 360.0), 360.0);
    ...
    double min_angle = fmod((rotation - field_of_view / 2.0 + 360.0), 360.0);
    double max_angle = fmod((rotation + field_of_view / 2.0 + 360.0), 360.0);
...
int main() {
    ...
    double pos1[2] = {-20.5, -0.5};  // Ihre Position
    double pos2[2] = {-4, -3.6};  // Position des Objekts
    double rotation = 0;   // Ihre Drehung in Grad
    double sichtfeld = 50; // Ihr Zeigebereich in Grad
