...
std::list<std::string> File::getSubDirs(std::string path)
{
  ...
  HANDLE finds = FindFirstFile(path.c_str(), &data);
  if (finds != INVALID_HANDLE_VALUE)
  {
    do
    {
      std::string dirName = data.cFileName;

      if ((data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && dirName != "." && dirName != "..")
      {
        subDirs.push_back(dirName);
      }
    } while (FindNextFile(finds, &data));
  }
