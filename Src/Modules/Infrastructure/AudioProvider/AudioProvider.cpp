...
void AudioProvider::update(AudioData& audioData)
{
  ...
  DEBUG_RESPONSE_ONCE("module:AudioProvider:checkChannels")
  {
  ...
  if (min_channels == max_channels) {
      OUTPUT_TEXT("The PCM device is set to use " << min_channels << " channels.");
    } else {
      OUTPUT_TEXT("The PCM device can use between " << min_channels << " and " << max_channels << " channels.");
    }
  }
