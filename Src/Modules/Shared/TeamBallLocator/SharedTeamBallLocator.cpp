/**
 * @file SharedTeamBallLocator.cpp
 *
...
#include "SharedTeamBallLocator.h"
#include "Tools/Modeling/BallPhysics.h"
#include "Tools/Settings.h"

MAKE_MODULE(SharedTeamBallLocator, modeling);


void SharedTeamBallLocator::update(TeamBallModel& teamBallModel)
{
...
void SharedTeamBallLocator::computeModel(TeamBallModel& teamBallModel)
{
...
void SharedTeamBallLocator::updateInternalBallBuffers()
{
...
void SharedTeamBallLocator::findAvailableBalls()
{
  for(unsigned int robot = 0; robot < balls.size(); ++robot)
  {
    if(balls[robot].size() > 2) // We want more than just the initial entry
    {
      ...
      {
        const SharedTeamBallLocator::BufferedBall& ball = balls[robot][i];
...
void SharedTeamBallLocator::clusterBalls()
{
...
float SharedTeamBallLocator::localizationQualityToModifier(const RobotPose::LocalizationQuality quality) const
{
...
float SharedTeamBallLocator::computeWeighting(const SharedTeamBallLocator::BufferedBall& ball) const
{
...
bool SharedTeamBallLocator::checkForResetByGameSituation()
{
  bool reset = false;
